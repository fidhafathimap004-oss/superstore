...
with tab1:
     ...
     monthly_year = (filtered.groupby([filtered['Order Date'].dt.to_period('M').astype(str),'Order Year'])['Sales'].sum().reset_index().rename(columns={'Order Date': 'Month'}))
     fig = px.line(monthly_year,x='Month', y='Sales',color='Order Year',title='Monthly Sales Trend by Year')
...
with tab4:
    ...
    if mean_margin < 10:
        st.error(f"Low profit margin: {mean_margin:.1f}% - investigate discounts and product mix.")
    elif mean_margin < 20:
...
st.caption(f"showing {len(filtered):,} Rows • {min_year}–{max_year} • Built by fidha")